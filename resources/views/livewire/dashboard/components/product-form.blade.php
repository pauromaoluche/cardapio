<form wire:submit.prevent="save">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="name" placeholder="Ex: X-Burger" wire:model.live="name">
        @error('name')
    ...
    <div class="mb-3">
        ...
        <input type="file" class="form-control" id="images" multiple wire:model="images">
            ...
            <div class="mt-3 d-flex flex-wrap gap-3 mb-3">
                @foreach ($product->images as $item)
                    {{-- <div class="position-relative">
                        <img src="{{ asset('storage/' . $item->path) }}" alt="Preview" class="img-thumbnail"
                            style="height: 200px;">
                        <button type="button" wire:click="removeImageTemporary({{ $item->id }})"
                            class="btn btn-sm btn-danger position-absolute top-0 end-0"
                            style="border-radius: 50%; width: 25px; height: 25px; padding: 0;">
                            &times;
                        </button>
                    </div> --}}
                    <x-image-preview :image="$item" path="{{ asset('storage/' . $item->path) }}" :key="$item->id"
                        removeMethod="toggleImageRemoval" :imagesToRemove="$imagesToRemove" isExistingImage />
                @endforeach
            </div>
        @endif
                ...
                @foreach ($images as $index => $image)
                    {{-- <div class="position-relative">
                        <img src="{{ $image->temporaryUrl() }}" alt="Preview" class="img-thumbnail"
                            style="height: 200px;">
                        <button type="button" wire:click="removeImageTemporary({{ $index }})"
                            class="btn btn-sm btn-danger position-absolute top-0 end-0"
                            style="border-radius: 50%; width: 25px; height: 25px; padding: 0;">
                            &times;
                        </button>
    ...
    <div class="d-flex justify-content-end">
        <div class="btn-group" role="group">
            ...
            <button type="button" class="btn btn-primary" wire:click="save(true)">Salvar e adicionar outro</button>
